...
- **板级支持包 (BSP)**: 硬件抽象层位于 main/bsp_*.c 和 main/include/bsp_*.h。
  - sp_display.c: SPI LCD 驱动和 UI 渲染逻辑。
  - sp_joystick.c: 基于 ADC 的摇杆输入处理及校准。
  - sp_key_fsm.c: 按键输入的有限状态机。
  - sp_dns.c / sp_http.c: 网络服务。
- **组件**:
...
- **任务管理**: 使用 FreeRTOS 任务和队列。示例：main/main.c 中的 eep_task 通过队列处理蜂鸣器输出。
- **内存管理**: sp_config.h 中的自定义宏 user_malloc 和 user_iram_malloc 封装了 heap_caps_malloc。
- **引脚定义**:
...
- **UI 更新**: 修改 sp_display.c 以更新帧缓冲区 (lcd_frame_buffer) 和菜单结构 (disp_menu_t)。
...
vs_set_*,
vs_commit 序列进行配置持久化。
- **ADC 校准**: sp_joystick.c 演示了 dc_cali_scheme_line_fitting 的用法。
