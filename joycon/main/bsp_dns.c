 ...
 */
 ...
 static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len, dns_server_handle_t h)
 {
     ...
     ESP_LOGD(__func__, "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d",
     ...
     for (int qd_i = 0; qd_i < qd_count; qd_i++)
     {
         ...
         {
             ESP_LOGE(__func__, "Failed to parse DNS question: %s", cur_qd_ptr);
             return -1;
         }
         ...
         ESP_LOGD(__func__, "Received type: %d | Class: %d | Question for: %s", qd_type, qd_class, name);
         ...
         if (qd_type == QD_TYPE_A)
         {
             ...
             ESP_LOGD(__func__, "Answer with PTR offset: 0x%" PRIX16 " and IP 0x%" PRIX32, ntohs(answer->ptr_offset), ip.addr);
 ...
 */
 void dns_server_task(void *pvParameters)
 {
     ...
     while (handle->started)
     {
         ...
         ESP_LOGI(__func__, "Socket bound, port %d", DNS_PORT);
 
         while (handle->started)
         {
             ESP_LOGI(__func__, "Waiting for data");
             ...
             if (len < 0)
             {
                 ESP_LOGE(__func__, "recvfrom failed: errno %d", errno);
                 close(sock);
                 break;
             }
             // Data received
             else
             {
                 ...
                 ESP_LOGI(__func__, "Received %d bytes from %s | DNS reply with len: %d", len, addr_str, reply_len);
                 if (reply_len <= 0)
                 {
                     ESP_LOGE(__func__, "Failed to prepare a DNS reply");
                 }
                 else
                 {
         ...
         if (sock != -1)
         {
             ESP_LOGE(__func__, "Shutting down socket");
             shutdown(sock, 0);
             close(sock);
         }
     }
 ...
 dns_server_handle_t start_dns_server(dns_server_config_t *config)
 {
     ...
     ESP_RETURN_ON_FALSE(handle, NULL, __func__, "Failed to allocate dns server handle");
